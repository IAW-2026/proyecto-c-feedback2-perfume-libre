...
import { Star, ChevronDown, ChevronUp, Flag, ArrowUpDown, ArrowUp, ArrowDown, X, Upload } from "lucide-react";
...
export default function SellerDashboard() {
  ...
  const handleReportOpen = (bundle: ReviewBundle) => {
    setReportBundle(bundle);
    setReportSuccess(false);
    setReportImages([]);
    setReportReason("INAPROPIADO");
    
    // Auto-select si solo hay una de las dos reseñas
    ...
    else setReportType(null); // Obligar a elegir si están ambas
  };

  const handleReportSubmit = async () => {
    if (!reportBundle || !reportType) return;
    
    const idResena = reportType === "PRODUCTO" ? reportBundle.resenaProducto?.idResena : reportBundle.resenaVendedor?.idResena;
    ...
    try {
      ...
      if (data.estado === "success") {
        ...
        setTimeout(() => setReportBundle(null), 2000);
      } else {
        alert(data.mensaje);
      }
    } catch (error) {
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-5xl mx-auto">
                      ...
                      <td colSpan={5} className="p-0 border-b border-gray-200">
                        <div className="p-6 bg-gray-50 flex flex-col gap-4 shadow-inner">
                          ...
                          <div className="mt-2 flex justify-end">
                            <button 
                              onClick={(e) => { e.stopPropagation(); handleReportOpen(bundle); }}
      ...
      {/* Report Modal */}
      {reportBundle && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4" onClick={() => !isReporting && setReportBundle(null)}>
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center p-4 border-b border-gray-200 bg-gray-50">
              ...
              <button onClick={() => setReportBundle(null)} className="text-gray-400 hover:text-gray-600" disabled={isReporting}>
                <X size={20} />
              </button>
            </div>
                ...
                <>
                  <div>
                    ...
                    <div className="flex flex-col gap-2">
                      <label className={`flex items-center gap-3 p-3 border rounded cursor-pointer transition-colors ${reportType === "PRODUCTO" ? "border-teal-700 bg-teal-50" : "border-gray-300"} ${!reportBundle.resenaProducto ? "opacity-50 cursor-not-allowed" : ""}`}>
                        <input 
                          type="radio" 
                          name="reportType" 
                          value="PRODUCTO" 
                          disabled={!reportBundle.resenaProducto}
                      ...
                      <label className={`flex items-center gap-3 p-3 border rounded cursor-pointer transition-colors ${reportType === "VENDEDOR" ? "border-teal-700 bg-teal-50" : "border-gray-300"} ${!reportBundle.resenaVendedor ? "opacity-50 cursor-not-allowed" : ""}`}>
                        <input 
                          type="radio" 
                          name="reportType" 
                          value="VENDEDOR" 
                          disabled={!reportBundle.resenaVendedor}
